...
var h$TH = { nWaiters: 0
           , waiters: {}
           , data: null
           , dataWritten: 0 // how much of data buffer is populated.
           , requestId: 1
           , loadedSymbol: null
...
// if we don't process.exit, the build hangs when there is an error
// if we do exit, console.log usually hasn't flushed and we
// don't get to see the error. ugh.
...
//                h$TH.data = h$TH.data ? Buffer.concat([h$TH.data, buf]) : buf;
                if(h$TH.data) {
                  var newdatalen = 0;
                  newdatalen += h$TH.dataWritten;
                  newdatalen += buf.length;

                  // increase buffer size if needed
                  if(newdatalen > h$TH.data.length) {
                    var newsize = newdatalen + Math.min(newdatalen,10000000); 
                    // can be anything >= newdatalen - choice of size affects performance not correctness
                    var newbuf = new Buffer(newsize);
                    h$TH.data.copy(newbuf);
                    h$TH.data = newbuf;
                  }

                  buf.copy(h$TH.data,h$TH.dataWritten);
                  h$TH.dataWritten += buf.length;
...
function h$processQueue() {
    while(h$TH.nWaiters > 0 && h$TH.data && h$TH.dataWritten >= 8) {
        var msgLength = h$TH.data.readUInt32BE(0);
        var msgTarget = h$TH.data.readUInt32BE(4);
        if(h$TH.dataWritten >= msgLength + 8 && h$TH.waiters[msgTarget]) {
            console.log("Processing queue (BENC)");
            var w = h$TH.waiters[msgTarget], b = h$TH.data.slice(8, msgLength + 8);
            h$TH.data = h$TH.data.slice(msgLength + 8);
            h$TH.dataWritten = h$TH.dataWritten - (msgLength + 8);
